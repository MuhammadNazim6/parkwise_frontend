...
function ProvBookings() {
  ...
  const fetchBookingsOfParkingLot = async () => {
    const data = { lotId: providerInfo.id, page }
    const response = await fetchBookingsOfLot(data).unwrap()
    if (response.success) {
        ...
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg h-screen">
          ...
          {(bookings && bookings.length) ? (
            <>
              ...
              <div className="fixed inset-x-0 bottom-5 md:ml-60 md:flex justify-center">
